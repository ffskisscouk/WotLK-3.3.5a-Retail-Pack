...
function lib:ITEM_DATA_LOAD_RESULT(event, item, success)
  -- print("### ITEM_DATA_LOAD_RESULT: " .. tostring(item) .. ", " .. tostring(success))
  ...
  while true do
    ...
    while true do
      ...
      elseif not itemRequestTimeoutAt[item] then
        ...
        if not self.frame:IsEventRegistered("ITEM_DATA_LOAD_RESULT") then
          self.frame:RegisterEvent("ITEM_DATA_LOAD_RESULT")
        end
        ItemEventListener:AddCallback(item, null)
        return true
...
function lib:initialOnUpdate()
  ...
  self.frame:Hide()
  self.frame:UnregisterEvent("ITEM_DATA_LOAD_RESULT")
end
